...
public class PhongDAO {
	...
	public List<Phong> getAllRooms() {
		...
		try (Connection conn = DBContext.getConnection();
			...
			while (rs.next()) {
				list.add(new Phong(rs.getInt("MaPhong"), rs.getString("TenPhong"), rs.getString("LoaiPhong"),
						rs.getInt("SoTang"), rs.getDouble("Gia"), rs.getString("TrangThaiPhong"),
						rs.getDouble("DienTich"), rs.getString("MoTa"), rs.getString("AnhPhong")));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Phong getRoomById(int id) {
		...
		try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
					return new Phong(rs.getInt("MaPhong"), rs.getString("TenPhong"), rs.getString("LoaiPhong"),
							rs.getInt("SoTang"), rs.getDouble("Gia"), rs.getString("TrangThaiPhong"),
							rs.getDouble("DienTich"), rs.getString("MoTa"), rs.getString("AnhPhong"));
				}
			}
	...
	public boolean updateRoom(Phong p) {
		String sql = "UPDATE PHONG SET TenPhong=?, LoaiPhong=?, SoTang=?, Gia=?, DienTich=?, MoTa=?, AnhPhong=?, TrangThaiPhong = ?"
				+ "WHERE MaPhong=?";
		try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			ps.setInt(9, p.getMaPhong());
			ps.setString(8, p.getTrangThaiPhong()); //thêm sửa trạng thái phòng	
	...
	public List<Phong> getRoomStatusByDate(java.sql.Date targetDate) {
		...
		try (Connection conn = DBContext.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					list.add(new Phong(rs.getInt("MaPhong"), rs.getString("TenPhong"), rs.getString("LoaiPhong"),
							rs.getInt("SoTang"), rs.getDouble("Gia"), rs.getString("TrangThaiDong"),
							rs.getDouble("DienTich"), rs.getString("MoTa"), rs.getString("AnhPhong")));
				}
			}
		} catch (Exception e) {
	...
	public Double GetRevenue() {
		Double total= 0.0;
		String sql = "SELECT SUM(Gia) FROM phong b WHERE b.TrangThaiPhong = 'CÓ KHÁCH'";
		try(Connection conn = DBContext.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery()){
			if(rs.next()) {
	...
	// hiện thị sô lượng người đang lưu trú
	public Double GetTotalGuests() {
		Double total= 0.0;
		String sql = "SELECT SUM(SoNguoi) FROM dondatphong b WHERE b.TrangThaiDon = 'ĐANG LƯU TRÚ'";
		try(Connection conn = DBContext.getConnection();
			PreparedStatement ps = conn.prepareStatement(sql);
			ResultSet rs = ps.executeQuery()){
			if(rs.next()) {
