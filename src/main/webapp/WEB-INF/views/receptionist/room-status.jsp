  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
    ...
    <div class="row mb-4 align-items-center">
        <div class="col-md-6">
            ...
            <p class="text-sm mb-0">Theo dõi trạng thái phòng theo thời gian thực</p>
        </div>
        
        <!-- BỘ CHỌN NGÀY -->
        <div class="col-md-6 d-flex justify-content-md-end mt-3 mt-md-0">
            <form action="${pageContext.request.contextPath}/receptionist/room-status" method="GET" class="d-flex align-items-center m-0">
                <label class="text-sm font-weight-bold mb-0 me-2 text-dark">Kiểm tra sơ đồ ngày:</label>
                <!-- onchange="this.form.submit()" giúp tự động tải lại trang khi đổi ngày -->
                <input type="date" name="checkDate" value="${currentDate}" 
                       class="form-control border px-2 py-1 bg-white" style="width: 180px; height: 35px;" 
      ...
      <div class="row mb-4">
          <div class="col-12 d-flex align-items-center">
              <span class="badge bg-gradient-success me-2">TRỐNG</span> Sẵn sàng đón khách
              <span class="badge bg-gradient-danger mx-2">CÓ KHÁCH</span> Đang được sử dụng
              <span class="badge bg-gradient-warning mx-2">BẢO TRÌ</span> Không thể đặt
          </div>
      </div>
      ...
      <div class="row">
          <c:forEach items="${listPhong}" var="phong">
              ...
              <c:if test="${phong.trangThaiPhong == 'CÓ KHÁCH'}">
                  ...
                  <c:set var="iconClass" value="text-white" />
              </c:if>
              ...
              <div class="col-xl-3 col-sm-6 mb-4">
                  <c:choose>
                      <c:when test="${phong.trangThaiPhong == 'TRỐNG'}">
                          <a href="${pageContext.request.contextPath}/receptionist/book-room?maPhong=${phong.maPhong}" style="text-decoration: none; display: block;">
                      </c:when>
                      <c:otherwise>
                      ...
                      <div class="card ${bgClass} ${phong.trangThaiPhong == 'TRỐNG' ? 'room-hover-effect' : ''}">
                          <div class="card-body p-3">
                              <div class="row">
                                  <div class="col-8">
                                      <div class="numbers">
                                          <p class="text-sm mb-0 text-capitalize font-weight-bold ${textClass}">
                                              Tầng ${phong.soTang} - ${phong.loaiPhong}
                                          </p>
                                          <h5 class="font-weight-bolder mb-0 ${textClass}">
                                              ${phong.tenPhong}
                                          </h5>
                                          <p class="text-xs mb-0 font-weight-bold mt-2 ${textClass}">
                                              <fmt:formatNumber value="${phong.gia}" type="number" /> đ/đêm
                                          </p>
                                      </div>
                                  </div>
                                  <div class="col-4 text-end">
                                      <div class="icon icon-shape bg-white shadow text-center border-radius-md d-flex align-items-center justify-content-center">
                                          <i class="material-icons opacity-10 ${iconClass} text-lg" aria-hidden="true">
                                              ${phong.trangThaiPhong == 'TRỐNG' ? 'check_circle' : 
                                                phong.trangThaiPhong == 'CÓ KHÁCH' ? 'bed' : 'build'}
                                          </i>
                                      </div>
                                  </div>
                              </div>
